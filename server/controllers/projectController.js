...
export const createProject = async(req, res)=>{
    try {
        ...
        // check if user has admin role for workspce
        const workspce = await prisma.workspace.findUnique({
        ...
        if (!workspce.members.some((member)=>member.userId === userId && member.role === "ADMIN")) {
        ...
        if (team_members?.length > 0) {
            const membersToAdd = []
            workspce.members.forEach(member=>{
...
export const addMember = async(req, res)=>{
    try {
       ...
       const project = await prisma.project.findUnique({
        ...
        //  chwck if user is already a member
